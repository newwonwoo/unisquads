...
def resolve_one_api(address: str, session: Optional[requests.Session] = None,
    ...
    def _query_direct(addr, d, h, bname="", page_unit=500):
        """동·호를 검색어에 넣어 좁게 조회(2026-07-18). 실패해도 확정하지 않는다."""
        payload = _build_payload(addr, dong=d, ho=h, buld_name=bname, page_unit=page_unit)
        try:
            r = s.post(SEARCH_API, json=payload,
                       headers={"Content-Type": "application/json; charset=UTF-8"},
                       timeout=timeout)
    ...
    try:
        # ── 동·호 직접 검색(2026-07-18) ──────────────────────────────
        # IROS 검색창은 키워드 검색이라 "지번 동 호"를 넣으면 해당 세대가
        # 바로 걸리는 경우가 많다(실측: 개포동 653은 지번만으로는 토지·건물
        # 등기만 오고 세대가 안 온다). 먼저 좁게 시도하고, 안 되면 아래
        # 기존 경로(지번 전체 수집 후 로컬 필터)로 폴백한다.
        # 주의: 대단지는 표기·색인 차이로 직접 검색이 누락될 수 있으므로
        # 여기서 0건이 나와도 절대 세대 없음으로 확정하지 않는다.
        if dong or ho:
            for _bn in ([buld_name] if buld_name else []) + [""]:
                _d0, _c0 = _query_direct(pure_lot_addr, dong, ho, _bn)
                if _d0 is None or (isinstance(_d0, dict) and "_raw" in _d0):
                    continue
                _cd0 = _parse_json_response(_d0)
                if not _cd0:
                    continue
                _hit = [x for x in _cd0 if _match_unit(x, dong, ho)]
                if len(_hit) == 1:
                    x = _hit[0]
                    desc = " ".join(filter(None, [x.get("gubun", ""), x.get("state", ""),
                                                  (x.get("dong") and f"{x['dong']}동") or "",
                                                  (x.get("ho") and f"{x['ho']}호") or ""]))
                    return ResolveResult(address, "RESOLVED", unique_no=x["unique_no"],
                                         candidates=_cd0, message=desc.strip() + " (동·호 직접검색)")

        # 1차: 순수 지번만(주소 자체에 실려온 건물명까지 제거) — 가장 빠르고
        # 안정적인 기본 경로. 여기서 받은 전체 후보를 로컬 필터로 좁힌다.
